def CameraError(df_vboxattendance,df_sitemas,df_cam):
    ...
    def transform(df_vboxattendance,df_sitemas,df_cam):
        ...
        cur2=conn2.cursor()
        if len(BankName)>1:
            df_opened_tickets=pd.read_sql("select TicketId,BankName,VboxId,CameraId,CameraName from Camera_Issues where BankName in {} and ClosedOn IS NULL".format(BankName),conn2)
        else:
            BankName=BankName[0]
            df_opened_tickets=pd.read_sql("select TicketId,BankName,VboxId,CameraId,CameraName from Camera_Issues where BankName='{}' and ClosedOn IS NULL".format(BankName),conn2)

        df_vboxattendance=df_vboxattendance[df_vboxattendance['AtTime']>=prior_30_mins]
        ...
        df_vboxattendance=df_vboxattendance.sort_values(['AtTime']).drop_duplicates(['Vboxid','DeviceId'],keep='last')
        ...
        df_vbox_latest=df_vboxattendance.sort_values(['OpenedOn']).drop_duplicates(['Vboxid'],keep='last')
        ...
        df_new_tickets=pd.merge(df_sitemas[['BankName','s_Name','McuId','VBoxId','s_Addr_State','s_Addr_District','s_Addr_Street','s_Addr_Zone']],df_new_tickets,left_on='VBoxId',right_on='Vboxid')
        df_new_tickets.rename(columns={'s_Name':'SiteName','Vboxid':'VboxId','s_Addr_State':'State','s_Addr_District':'District','s_Addr_Street':'Address','s_Addr_Zone':'Zone'},inplace=True)
        ...
        df_new_tickets=df_new_tickets[['BankName','SiteName','McuId','VBoxId','State','District','Address','Zone','CameraId','CameraName','OpenedOn']]
