def Freq_NC30(ServerAddress,UserName,Password,DatabaseName):
	...
	def fetch(ServerAddress,UserName,Password,DatabaseName):
		...
		df_sitemas=pd.read_sql("select a.s_Name,a.McuId,b.BankName from lbtSiteMas as a, lbtBankMas as b WHERE a.BankCode=b.BankCode AND a.s_Status=1 AND a.qstatus='True' AND b.qstatus='True' ",conn)
		cursor.close()
		conn.close()
	...
	def transformation(events,df_sitemas,today,prior3days,prior7days):
		...
		events=pd.merge(df_sitemas[['BankName','s_Name','McuId']],events,on='McuId',how='left')
		...
		Events=Events[['BankName','s_Name','McuId','EventId','Msg','AtTime','OpenedOn','ClosedOn']]
		...
		pattern_last_15days=pattern_last_15days[pattern_last_15days['count_15days']>=8]
		...
		pattern_last_7days=pattern_last_7days[pattern_last_7days['count_7days']>=4]
		...
		pattern_last_3days=pattern_last_3days[pattern_last_3days['count_3days']>=2]
		...
		df_business_hour_pattern_events=pd.merge(df_sitemas[['BankName','SiteName','McuId']],df_business_hour_pattern_events,on='McuId')
		...
		return df_offline_online_pattern,df_event_count,df_business_hour_pattern_events,df_offline_last3days,df_offline_last7days,df_offline_allday#,df_daily_offline
	def pushdata(df_offline_online_pattern,df_event_count,df_business_hour_pattern_events):
		if (len(df_offline_online_pattern)+len(df_event_count)+len(df_business_hour_pattern_events))>0:
			...
			df_event_count.to_sql('freq_nc30',engine,if_exists='append',index=False, method="multi", chunksize=500)
			df_offline_online_pattern.to_sql('freq_nc30',engine,if_exists='append',index=False)
			df_business_hour_pattern_events.to_sql('freq_nc30',engine,if_exists='append',index=False) 
			#df_daily_offline.to_sql('freq_nc30',engine,if_exists='append',index=False) 
			engine.dispose()
		else :
			pass
				
	events,df_sitemas,today,prior3days,prior7days=fetch(ServerAddress,UserName,Password,DatabaseName)
	df_offline_online_pattern,df_event_count,df_business_hour_pattern_events,df_offline_last3days,df_offline_last7days,df_offline_allday=transformation(events,df_sitemas,today,prior3days,prior7days)
